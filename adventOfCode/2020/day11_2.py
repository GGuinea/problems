...
while True:
    ...
    for r in range(ROW):
        for c in range(COLUMN):
            ...
            for dr in [-1,0,1]: # three values in loop for row, actual(0) up and down
                for dc in [-1,0,1]: # same as above but for coulumn
                    if not (dr == 0 and dc ==0):  # we are not intreseted in your current possiton
                        ...
                        while 0 <= rr < ROW and 0 <= cc < COLUMN and data[rr][cc] == ".": # skipping floor for part2
                            rr = rr+dr #skipping floor in row direction
                            cc = cc+dc # skipping floor in column diredction 
            ...
            elif data[r][c] == "#" and occupied >= 5: # checking if current postiion is occupied, and if has many neigbourhoods
